...
# This script implements the firewall created through the config file
# and the firewall-policy.py file.
...
def parse_config(filename):
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            if (ipaddr_src != '-' and None == re.match("^(25[0-5]|2[0-4]\d|[01]\d{2}|\d{1,2})(\.(25[0-5]|2[0-4]\d|[01]\d{2}|\d{1,2})){3}(/(3[012]|[12]\d|\d))$",ipaddr_src)):
                raise TypeError("Source IP Address for rulenum %s is invalid" % rulenum)                
            if (ipaddr_dst != '-' and None == re.match("^(25[0-5]|2[0-4]\d|[01]\d{2}|\d{1,2})(\.(25[0-5]|2[0-4]\d|[01]\d{2}|\d{1,2})){3}(/(3[012]|[12]\d|\d))$",ipaddr_dst)):
            ...
            if (protocol != '-' and (protocol not in ('T','U','I','O'))):
                raise TypeError("protocol for rulenum %s is invalid (seeing protocol %s)" % (rulenum,protocol))  
            if (ipproto != '-' and (int(ipproto) > 255 or int(ipproto) < 1)):
                raise TypeError("Other IP Protocol for rulenum %s is invalid" % rulenum)  
